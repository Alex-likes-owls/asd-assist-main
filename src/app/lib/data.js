let cardData = [
  {
    ...
    article:
      "Autism being caused by vaccines is a pervasive myth on the internet. However, this is easily disproven with the wealth of scientific research on the disorder, proving that you are born with the disorder and cannot get it later in life. Another popular myth is that all autistic individuals show the same symptoms and act the same. Autism is a spectrum, so some people exhibit some characteristics more and some less. Thirdly, the autistic sociopath stereotype. Contrary to what the media shows, autistic individuals are capable of feeling empathy. Some individuals even have hyper-empathy. Autism is a neurological variation rather than a disease, so it does not have a 'cure.' People may act more 'normal' as they grow up, but the condition will remain. Lastly, not all autistic individuals are geniuses or have intellectual disabilities. Savant syndrome, or when a person possesses extraordinary abilities, is rare to occur, and the same could be said for intellectual disabilities as well. Each autistic person is different, so one cannot form an opinion on them as a whole because of a single person.",
  },
  {
    id: 1,
    ...
    article:
      " Hyperfixations are defined as when an individual shows interest in a chosen object, activity, or person for a prolonged period to an extreme degree. Individuals may use hyperfixations to cope with negative emotions as they relish learning more about their chosen thing. For example, a person may spend time learning about their favorite characters in a show instead of doing schoolwork. Eating the same meals for a long time can be considered a hyperfixation as you hyperfixate on the dish. You may think that hyperfixations are simply obsessions with a fancier name, but you would be wrong, as these two have two distinct differences. Hyperfixations come in waves and are enjoyable, whereas obsessions are not. A person may spend days eating the same meal, but one day, they might not like it anymore, and after some time, they might begin to like it again and hyperfixate on it as they enjoy eating the food. However, this would not be the same for obsessions, as the individual would not get the same joy and may think they 'have to' eat the meal. Hyperfixatons are also not exclusive to neurodivergents as neurotypicals experience them too.",
  },
  {
    id: 2,
    title: "Stimming",
